 ...
 * 
 * @author jmac
 * @version August 2014. v3 released 9/3/14.
 */
public class SimpleCannibals {
	...
	private String findSolution(int num_cannibals) {
		...
		State initial_state = new State(num_cannibals, num_cannibals, 0, 0,
				BANK.Left);
		Node initial_node = new Node(initial_state, null, null, 0);
		...
		if (initial_node.state.isGoal()) {
			return initial_node.pathDetails();
		}
		...
		// Enter a loop that, in each iteration, expands the head node
		// of the frontier. Each child of the expanded node is first
		// checked to see if it contains a goal state, and if so, the
		// solution is returned. Otherwise, the child must be added to
		// the frontier for exploration later (unless its state has
		// been explored already, or is in the frontier already).
		while (true) {
			if (frontier.size() == 0) {
				return "no solution";
			}
